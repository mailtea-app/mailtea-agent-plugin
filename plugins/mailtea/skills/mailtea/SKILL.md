...
In Codex, install the Mailtea plugin from the Mailtea repository marketplace
and connect the hosted MCP server through the client's OAuth flow. Sign in to
Mailtea and choose the publication and permissions on the consent screen.
Do not ask users to paste credentials into the conversation.

For a local stdio connection, get a personal access token (prefix `mt_pat_`) from the Mailtea dashboard
